const scrollspy_progress = document.getElementById(
  "hero-scrollspy-progress-element"
);
...
function update_scrollspy_progress_top_value(new_top: number) {
...
window.addEventListener("scroll", function () {
  const current_scroll_position =
    this.window.scrollY + this.window.innerHeight / 2;
  ...
  sections.forEach((current_section, current_section_index) => {
    if (
      ...
      const scrollspy_progress_top_value = parseInt(
        (scrollspy_links[current_section_index] as HTMLElement).dataset
          .topValueForProgressEl || "0",
        10
      );
      update_scrollspy_progress_top_value(scrollspy_progress_top_value);
    }

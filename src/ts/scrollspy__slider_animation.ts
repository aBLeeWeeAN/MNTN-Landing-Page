...
scrollspy_links.forEach((link) => {
    link.addEventListener("click", (e) => {
        ...
        const scrollspy_progress_top_value = parseInt((link as HTMLElement).dataset.topValueForProgressEl || "0", 10);
    ...
    sections.forEach((current_section, current_section_index) => {
        if (
            ...
            const scrollspy_progress_top_value = parseInt(
                (scrollspy_links[current_section_index] as HTMLElement).dataset.topValueForProgressEl || "0",
                10
            );
            update_scrollspy_progress_top_value(scrollspy_progress_top_value);
        }
